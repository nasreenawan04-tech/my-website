...
const PDFHeaderFooterGenerator = () => {
  ...
  const handleAddHeaderFooter = async () => {
    ...
    if (!headerText && !footerText && !includePageNumbers && !includeDates) {
      alert('Please add some content for headers or footers.');
      return;
    }
    ...
    try {
      ...
      formData.append('includeDates', includeDates.toString());
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${selectedFile.name.replace('.pdf', '')}_with_header_footer.pdf`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
      } else {
        throw new Error('Failed to add header/footer to PDF');
      }
    } catch (error) {
      console.error('Error adding header/footer to PDF:', error);
      alert('An error occurred while adding header/footer to the PDF. Please try again.');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>PDF Header/Footer Generator - Add Custom Headers and Footers | ToolsHub</title>
        <meta name="description" content="Add custom headers and footers with page numbers and dates to PDF documents. Free online PDF header footer tool." />
        <meta name="keywords" content="PDF header footer, add PDF headers, PDF page numbers, PDF footers, custom PDF headers" />
      </Helmet>
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 bg-neutral-50">
          <section className="bg-gradient-to-r from-indigo-600 via-indigo-500 to-blue-700 text-white py-16">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
              <div className="w-20 h-20 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center mx-auto mb-6">
                <i className="fas fa-heading text-3xl"></i>
              </div>
              <h1 className="text-4xl sm:text-5xl font-bold mb-4">
                PDF Header/Footer Generator
              </h1>
              <p className="text-xl text-indigo-100 mb-8 max-w-2xl mx-auto">
                Add custom headers and footers with page numbers and dates to your PDF documents
              </p>
            </div>
          </section>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-3 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Enter header text (optional)"
                        className="w-full p-3 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                        rows={3}
                      />
                    </div>
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center">
                          <input
                            ...
                            className="w-4 h-4 text-indigo-600 bg-neutral-100 border-neutral-300 rounded focus:ring-indigo-500"
                          />
                          <label htmlFor="includePageNumbers" className="ml-2 text-sm text-neutral-700">
                            Include page numbers
                          </label>
                        </div>
                        <div className="flex items-center">
                          <input
                            ...
                            className="w-4 h-4 text-indigo-600 bg-neutral-100 border-neutral-300 rounded focus:ring-indigo-500"
                          />
                          <label htmlFor="includeDates" className="ml-2 text-sm text-neutral-700">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-6">
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-indigo-600 to-blue-700 text-white px-6 py-3 rounded-lg font-semibold hover:from-indigo-700 hover:to-blue-800 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isProcessing ? (
                        <>
